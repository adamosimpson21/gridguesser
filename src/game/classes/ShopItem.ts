...
export default class ShopItem{
    ...
    playerCanUseItemCheck(item: shopItemType):boolean{
        ...
        if(itemEffect.heal >= 0 && GameState.player.hp === GameState.player.maxHp){
            ...
            EventBus.emit(UI_EVENTS.DISPLAY_MESSAGE, {type: UI_EVENTS.ILLEGAL_ACTION, message: `You're already have ${item.name}`}, 3000)
            return false;
        }
        return true;
    }
        ...
        Object.keys(itemEffect).forEach(effect => {
            switch(effect){
                ...
                case 'damage_reduce':
                    ...
                    EventBus.emit(UI_EVENTS.DISPLAY_MESSAGE, 'Damage Reduction Increased')
                    break;
                case 'fightGridExpand':
                    ...
                    EventBus.emit(UI_EVENTS.DISPLAY_MESSAGE, 'Room size width and height increased')
                    break;
                case 'bombNumberReduce':
                    GameState.bombNum -= itemEffect.bombNumberReduce;
                    EventBus.emit(UI_EVENTS.DISPLAY_MESSAGE, 'Reduced Bomb Number')
                    break;
                case 'fightGoldIncrease':
                    GameState.fightGoldReward += itemEffect.fightGoldIncrease;
                    EventBus.emit(UI_EVENTS.DISPLAY_MESSAGE, 'Increased Room reward Gold')
                    break;
